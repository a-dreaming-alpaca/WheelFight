...
主窗口会显示中央 ROI、中心线、实时分类、置信度、两种颜色占比和红色 X 分数；另外两个
窗口分别显示黄绿色和红色掩膜。红色候选框中的两条辅助对角线用于观察 X 是否对齐。
判定时要求 X 的四条斜臂及交叉中心都存在，并排除对角线外大面积铺满的红色。
...
减益图案和场地红框分别标定 `min_red_x_score`。画面上的颜色比例以百分数显示，例如
`1.500%` 对应配置值 `0.015`；场地红框必须保持 `Red X: NO`。
